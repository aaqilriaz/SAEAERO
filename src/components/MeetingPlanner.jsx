...
function Drawing({ d, r }) {
  ...
  return <svg className="aircraft" viewBox="0 0 600 340" role="img" aria-label="Scaled preliminary wing and conventional tail planform; packaging is schematic">
    <defs><pattern id="grid" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="#303a43" strokeWidth=".6" /></pattern></defs>
    <rect width="600" height="340" fill="url(#grid)" />
    <line x1="300" y1="10" x2="300" y2="325" stroke="#677968" strokeDasharray="5 5" />
    <rect x={300 - d.bayWidthIn * scale / 2} y="55" width={d.bayWidthIn * scale} height={d.lengthIn * scale} rx="15" fill="#222c2a" stroke="#85947e" />
    <path d={`M ${300-b/2} ${90-tip/4} L 300 ${90-root/4} L ${300+b/2} ${90-tip/4} L ${300+b/2} ${90+tip*0.75} L 300 ${90+root*0.75} L ${300-b/2} ${90+tip*0.75} Z`} fill="#34472f" fillOpacity=".9" stroke="#bbdba1" strokeWidth="1.5" />
    <rect x={300-tailB/2} y={tailY-tailC/4} width={tailB} height={tailC} fill="#34472f" stroke="#bbdba1" />
    {[...Array(d.motorCount)].map((_, i) => { const x = 300 + (i - (d.motorCount-1)/2) * b / (d.motorCount+1); return <g key={i}><rect x={x-4} y={65-root/4} width="8" height="24" rx="3" fill="#b5ce9d"/><line x1={x-d.propDiameterIn*scale/2} x2={x+d.propDiameterIn*scale/2} y1={65-root/4} y2={65-root/4} stroke="#b5ce9d" strokeWidth="2" /></g>; })}
    <circle cx="300" cy={cg} r="5" fill="#ec9a43" stroke="#744711"/><text x="312" y={cg+4} className="svglabel">Loaded CG</text>
    <path d={`M ${300-b/2} 20 V 30 H ${300+b/2} V 20`} fill="none" stroke="#7b906c"/><text x="300" y="22" textAnchor="middle" className="svglabel">{fmt(d.spanIn)} in span</text>
...
export default function MeetingPlanner({ initialDesign, onDesignChange }) {
  const [d, setD] = useState(() => { const saved = initialDesign ?? read('sae-design-2027-v1', {}); return { ...fresh(), ...saved, mass: { ...DEFAULT_DESIGN.mass, ...saved.mass }, massStationsIn: { ...DEFAULT_DESIGN.massStationsIn, ...saved.massStationsIn } }; });
  ...
  useEffect(() => { onDesignChange?.(d); }, [d, onDesignChange]);
  ...
  return <div className="planner">
    <header className="topbar"><div className="advanced-label">Advanced workspace<small>Custom inputs, comparisons and competition tools</small></div><div className="top-actions"><span className="class-pill">REGULAR CLASS · v2027.0</span><button onClick={() => window.print()}>Print brief</button><button className="primary" onClick={save} disabled={!r.valid}>+ Save scenario</button></div></header>
    <div className="workspace">
      ...
      <aside className={`inputs ${mobileInputs ? 'mobile-open' : ''}`}>
        ...
        <details><summary><b>01</b> Mission & bottles</summary><div className="fields">
        ...
        <details><summary><b>02</b> Wing & airfoil</summary><div className="fields">
        ...
        <details><summary><b>04</b> Cargo packaging</summary><div className="fields">
          ...
          {field('bottleLengthIn', 'Bottle overall length', 'in')}{field('bottleDiameterIn', 'Bottle maximum diameter', 'in')}<Field label="Between-bottle gap" value={d.bottleClearanceIn * 25.4} unit="mm" step={0.5} min={0} onChange={v => set('bottleClearanceIn', v / 25.4)}/>
        ...
        <details><summary><b>05</b> Propulsion & batteries</summary><div className="fields">
          ...
          {field('staticThrustN', 'Available static thrust (all motors)', 'N')}{field('maxPowerW', 'Available electrical power (all motors)', 'W', 10)}
      ...
      <main>
        <div className="page-heading"><div><span className="eyebrow">ADVANCED / REGULAR CLASS</span><h1>Refine the design.</h1><p>Adjust assumptions, compare configurations and check the competition requirements.</p></div><div className="rule-stamp"><strong>72 &lt; span &lt; 96</strong><span>inches · 2027 Regular Class</span><small>2 or 4 motors / 4S / ≤2200 mAh</small></div></div>
